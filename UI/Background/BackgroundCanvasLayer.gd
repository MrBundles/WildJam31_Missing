...
func _ready():
	...
	$BackgroundCPUParticles2D.global_position = Vector2(get_viewport().get_visible_rect().size.x / 2, -500)
	$DebrisCPUParticles2D.emission_rect_extents = get_viewport().get_visible_rect().size
